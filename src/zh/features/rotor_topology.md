...
## 4. 电励磁电机转子凸极式
对于凸极式电励磁电机转子，首先令转子 $d$ 轴位于 $y$ 轴，坐标原点位于电机几何中心 $O$，建立笛卡尔坐标系。令 $x$ 轴为极坐标的角度坐标为 $0^\circ$ 的位置，建立相应的极坐标系。凸极式电励磁电机转子参数化模型示意图如图 4-1 所示，以下计算涉及到的点线均为转子 $d$ 轴与 $x$ 轴之间的半极模型上的点。

根据圆弧圆心与原点的距离 $pso$（为叙述方便，正文中、图中的变量名称与代码的变量命名不统一，而是在表 4-1 中将其一一对应）及极尖半径 $psr$ 确定位于 $y$ 轴上的 $A_{5}$ 点，根据齿宽度 $pw$ 及转子齿靴深度 $ptw$ 得到 $A_{1}$ 点的横坐标 $x_{A1}$，且 $A_{1}$ 点位于以 $O2$ 为圆心、以 $psr$ 为半径的圆上，因此，$A_{1}$ 点的纵坐标：
$$
\
...
图 4-1 凸极式电励磁电机转子参数化模型示意图  

表 4-1 凸极式电励磁电机转子变量名称对应表
...
## 5. 电励磁电机转子平行槽
对于平行槽电励磁电机转子，首先令转子 $q$ 轴位于 $x$ 轴，坐标原点位于电机几何中心 $O$，建立笛卡尔坐标系。凸极式电励磁电机转子参数化模型示意图如图 5-1 所示，以下计算涉及到的点线均为转子 $d$ 轴与 $x$ 轴之间的半极模型上的点。

根据极尖半径 $p_{sr}$（为叙述方便，正文中、图中的变量名称与代码的变量命名不统一，而是在表 5-1 中将其一一对应）与转子槽深度 $p_{sd}$ 确定在 $x$ 轴上的 $A_1$ 点坐标：
$$
\
\left\{
\begin{aligned}
...
图 5-1 平行槽电励磁电机转子参数化模型示意图

表 5-1 平行槽电励磁电机转子变量名称对应表
...
## 6. 电励磁电机转子平行齿
对于平行齿电励磁电机转子，首先令转子 $d$ 轴位于 $y$ 轴，坐标原点位于电机几何中心 $O$，建立笛卡尔坐标系。令 $x$ 轴为极坐标的角度坐标为 $0^\circ$ 的位置，建立相应的极坐标系。凸极式电励磁电机转子参数化模型示意图如图 6-1 所示，以下计算涉及到的点线均为转子 $d$ 轴与 $x$ 轴之间的半极模型上的点。

根据圆弧圆心与原点的距离 $pso$（为叙述方便，正文中、图中的变量名称与代码的变量命名不统一，而是在表 6-1 中将其一一对应）及极尖半径 $psr$ 确定位于 $y$ 轴上的 $A_1$ 点，根据齿宽度 $pw$ 及转子齿靴宽度 $ptr$ 得到 $A_2$ 点的横坐标 $x_{A2}$，且 $A_2$ 点位于以 $O_2$ 为圆心、以 $psr$ 为半径的圆上，因此，$A_2$ 点的纵坐标：
$$
...
图 6-1 平行齿电励磁电机转子参数化模型示意图

表 6-1 平行齿电励磁电机转子变量名称对应表
